# Olá, eu sou o Marcelo Poliato 👋

### Product Manager com 30 anos de experiência em tecnologia — em transição estratégica para IA aplicada a negócios

Combino uma trajetória sólida em gestão de produto, liderança de squads e automação de processos com uma imersão prática em **Inteligência Artificial Generativa**, **LLMs** e o método **Vibe Coding** — transformando problemas reais de negócio em soluções inteligentes e escaláveis.

> *"Meu diferencial é traduzir a complexidade da IA em valor de negócio mensurável."*

---

## 🚀 O que estou construindo agora

Estou em transição ativa para a área de **IA & LLMs**, aplicando na prática ferramentas de ponta para desenvolver soluções de produto com IA. Meu foco está em:

- **Product Management para IA** — definição de escopo, roadmap e entrega de produtos com IA embarcada
- **Vibe Coding** — desenvolvimento ágil de aplicações com assistência de IA (Lovable, Cursor, Copilot)
- **Automação Inteligente** — uso de LLMs e engenharia de prompts para automatizar fluxos de negócio
- **Engenharia de Prompts** — criação de prompts estruturados para sistemas de IA em produção

---

## 🛠️ Tech Stack de IA

| Ferramenta | Uso nos Projetos |
|---|---|
| 🤖 Google AI Studio (Gemini) | Construção de sistemas CRM com IA generativa |
| 🧠 Claude (Anthropic) | Assistente de produto e automação de raciocínio |
| ☁️ AWS SageMaker Canvas | Predição e modelos de ML sem código |
| 📓 NotebookLM | Engenharia de prompts e aprendizado com fontes |
| ⚡ Lovable + Copilot | Vibe Coding — desenvolvimento de apps com IA |
| 🐘 PostgreSQL + DBeaver | Consultas e extração de dados |
| 📊 Power BI + MicroStrategy | Dashboards gerenciais e KPIs |

---

## 📂 Projetos em Destaque

### 🤖 [Sistema CRM com Google AI Studio](https://github.com/poliato2015-max/projeto_ia_google_ia_studio_sistema_crm)
Construção de um sistema de CRM inteligente utilizando Gemini via Google AI Studio. O projeto explora como LLMs podem automatizar interações e enriquecer dados de clientes em tempo real.
`TypeScript` `Google AI Studio` `Gemini` `LLM`

---

### 💸 [Copiloto de Vendas para Loja Gamer](https://github.com/poliato2015-max/projeto_ia_lovable_claude_app_venda_gamer)
App de vendas com IA que gera estratégias completas de venda, cross-sell e follow-up para WhatsApp em tempo real. Desenvolvido com Vibe Coding usando Lovable + Claude.
`TypeScript` `Lovable` `Claude` `Vibe Coding`

---

### 📱 [App de Finanças Pessoais com IA](https://github.com/poliato2015-max/projeto_ia_lovable_claude_copilot_app_financas)
Aplicação de gestão financeira pessoal desenvolvida com IA generativa e Vibe Coding — da ideia ao produto funcional com velocidade e inteligência.
`TypeScript` `Lovable` `GitHub Copilot` `Vibe Coding`

---

### 📘 [Investidor Iniciante em Renda Fixa com NotebookLM](https://github.com/poliato2015-max/projeto_ia_notebooklm_iniciante_investidor_renda_fixa)
Caderno temático que documenta o uso de IA (NotebookLM + engenharia de prompts) para aprender sobre Renda Fixa a partir de fontes oficiais (Tesouro Direto, Banco Central, CVM). Inclui 6 blocos de prompts comparativos, cicatrizes de aprendizado e miniguia completo.
`NotebookLM` `Engenharia de Prompts` `IA Aplicada` `Educação Financeira`

---

### ☁️ [Previsão de Estoque com AWS SageMaker Canvas](https://github.com/poliato2015-max/lab-aws-sagemaker-canvas-estoque)
Experimento com ML no-code para previsão de demanda de estoque usando AWS SageMaker Canvas — explorando como líderes de produto podem validar hipóteses com machine learning sem código.
`AWS SageMaker` `Machine Learning` `No-Code ML`

---

## 💼 Trajetória Profissional

```
30 anos em tecnologia
│
├── Telerisco (2018–2025) ─── Coordenador de Produto
│   ├── Liderança de squads multifuncionais
│   ├── Automação de atendimento via chatbot (WhatsApp)
│   ├── Homologação de sistemas em nuvem (AWS)
│   └── Gestão de integrações via webservices (Postman + Jira)
│
└── Pamcary (1995–2018) ──── Analista de Produto → Analista de Operações
    ├── Dashboards gerenciais em MicroStrategy
    ├── Automações em VBA/Excel
    └── Gestão de riscos logísticos e seguros de transporte
```

---

## 🎯 O que busco agora

Estou buscando oportunidades como **Product Manager de IA** em empresas que acreditam que a inteligência artificial deve gerar impacto real de negócio — não apenas inovar por inovar.

Se você trabalha com IA, automação inteligente ou produtos de dados e precisa de alguém que entende tanto de tecnologia quanto de negócio, vamos conversar.

---

## 📬 Conecte-se comigo

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Marcelo%20Poliato-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/marcelo-poliato)
[![GitHub](https://img.shields.io/badge/GitHub-poliato2015--max-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/poliato2015-max)

---

*Construindo na interseção entre Produto, IA e impacto real de negócio* 🤖
